...
function Redirect({reason}){
  ...
  const unchosenSeats = [...seatsDep, ...seatsArr].map((el) => el.seats).flat().filter((item) => !item.passengerId);
  ...
  const redirectToPassengersSelection = redirectMaker(
    'Для последующих действий нужно вести данные всех пассажиров',
    'Выбрать пассажиров',
    links.passengers
  );
  ...
  const redirectToMainFromSuccess = redirectMaker(
    'Пожалуйста, для последующих действий перейдите на главную страницу',
    'На главную',
    links.main
  );
