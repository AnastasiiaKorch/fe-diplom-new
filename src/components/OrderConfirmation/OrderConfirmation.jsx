...
import {selectPassengers, removePassenger} from "../../store/slices/passengersSlice";
import {selectResponse, selectLoading, selectError} from "../../store/slices/orderConfirmationSlice";
...
import passengerTypes from "../SeatsSelection/SeatsSelectionBlock/passengerTypes";
...
function OrderConfirmation(){
  ...
  const editBtnMaker = (link)=> (
    <button type="button" onClick={()=> onEditBtnClick(link)} className={styles.buttonEdit}>
      Изменить
    </button>
  );
    ...
    const seatDataConstructor = (el)=> {
      ...
      return{
        ...
        person_info: {
          ...
          gender: pas[pasFieldNames?.gender],
          birthday: `${date[2]}-${date[1]}-${0}`,
          document_type:
  ...
  const trainCard = (
    <div className={styles.card}>
      ...
      <TrainCard ticket={trainsOptions[selectedTrainIndex]?.ticket}
