...
function PassengersRow({number, sum, type, className}){
  const adultWord = number === 1 ? 'Взрослый' : 'Взрослых';
  const childrenWord = childrenFormatter(number);
  const word = type === passengerTypes.adults? adultWord : childrenWord;
  ...
  return(
    <div className={className}>
      ...
      <div className={styles.sum}>
        <span>{sum}</span>
        <img className={styles.currency} src={rub} alt='иконка-руб'/>
      </div>
    </div>
  );
}
