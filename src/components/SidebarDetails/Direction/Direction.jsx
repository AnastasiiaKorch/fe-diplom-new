...
import {datetimeToTime, datetimeToDate, secsToTime} from "../../../utils/timeFormatters";
...
function Direction({direction}){
  ...
  const arrow = direction === directions.departure ? arrowDeparture : arrowArrival
      ...
      <div className={styles.directions}>
        ...
        <div className={styles.bottom}>
          <div className={styles.direction}>
            ...
            <span className={styles.station}>{train?.to?.railway_station_name}</span>
    ...
    <>
      <div className={styles.wrapperTop}>
        <h4 className={styles.header}>
          <img src={img} alt={`стрелка-направление - ${directionRus}`}/>
        ...
        <button className={styles.button} type='button' onClick={clickHandler}>
          <img className={styles.expand} src={expandIcon} alt='иконка управления'/>
        </button>
      </div>
      {expanded && exp}
    </>
